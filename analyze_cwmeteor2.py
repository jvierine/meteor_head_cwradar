...
h=h5py.File("/data0/interstellar_2023/And-Sau/ch000/rf@1690408920.000.h5","r")
print(h.keys())
zz=h["rf_data"][:,0]
...
#b=d.get_bounds(c[0])
N_per_seg=1000
N_segments=int(len(zz)/(N_per_seg*code_len))
print(N_segments)
r_max=600
Nr=600
for si in range(rank,N_segments,size):
    ...
    # all channels
    z=zz[ (si*code_len*N_per_seg): (si*code_len*N_per_seg + N_per_seg*code_len) ]
...
#    plt.title("%s-%s"%(stuffr.unix2datestr( (si*code_len*N_per_seg+b[0])/100e3),
 #                      stuffr.unix2datestr( ((si+1)*code_len*N_per_seg+b[0])/100e3)))
    ...
    plt.pcolormesh(10.0*n.log10(n.transpose(n.abs(RTI))),vmin=-3,vmax=10)
    plt.colorbar()
    plt.subplot(212)
