...
#    nfloor=n.nanmedian(dB)
    ...
    ax0.set_title("Meteor head echo Doppler spectrum (%s)"%(title))
    ...
    ax11.set_ylabel("TX-%s range (km)"%(title),color="C1")
...
def range_doppler_matched_filter2(fname,         # data file name
                                 rg=[50,120],   # range ranges to analyze (in data samples)
                                 i0=35*100000,  # first sample to process
                                 i1=120*100000, # last sample to process
                                 step=500,      # how many samples to step in time
...
#    ,rmodel=p["rmodel"],dmodel=p["dmodel"],t_model=t_model
    ...
    llh=h["model_lat_lon_h"][()]
    h.close()
    hpolyt0=n.median(m_t)
    hpolycoef=n.polyfit(m_t-hpolyt0,llh[:,2]/1e3,5)
    hpoly=n.poly1d(hpolycoef)
#    si.interp1d
    plt.plot(tnows,hpoly(tnows-hpolyt0),".")
    plt.show()

    fig,ax0=plt.subplots(1,1,figsize=(8,4.5))
    #plt.subplot(122)
    s=ax0.pcolormesh(hpoly(tnows-hpolyt0),freqw,dB-nfloor,vmin=0)
    ho=h5py.File("head_doppler_spec_%s.h5"%(title),"w")
    ho["snr_dB"]=dB-nfloor
    ho["height"]=hpoly(tnows-hpolyt0)
    ho["doppler_Hz"]=freqw
    ho["time_unix"]=tnows
    ho["rx"]=title
    ho.close()
    ax0.set_xlabel("Height above ground (km)")
    ax0.set_title("Meteor head echo Doppler spectrum (%s)"%(title))
    ax0.set_ylabel("Doppler shift relative to model (Hz)")
    cb=fig.colorbar(s,ax=ax0,location="right")
    cb.set_label("SNR (dB)")
#    ax1.plot(m_t-tidx0/100e3,llh[:,2]/1e3,color="C0")
#    ax1.set_ylabel("Height (km)",color="C0")
#    ax1.grid()
...
#        ps=range_doppler_matched_filter(p["fname"],plot=True,title=p["title"],plot_file=p["plot_file"],rmodel=p["rmodel"],dmodel=p["dmodel"],t_model=t_model)
        ...
        ps=range_doppler_matched_filter2(p["fname"],plot=True,title=p["title"],plot_file=p["plot_file"],rmodel=p["rmodel"],dmodel=p["dmodel"],t_model=t_model)
